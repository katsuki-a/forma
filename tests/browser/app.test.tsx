...
function setup() {
  ...
  const transport: typeof fetch = async (input, init) =>
    api.request(new Request(input, init));
...
test("FRM-001/002/010/011/016 空の作成から項目編集、反映、CRUD、再利用、一括削除まで", async () => {
  ...
  await expect
    .element(page.getByRole("cell", { name: "牛乳を買う", exact: true }))
    .toBeVisible();
  await page.getByRole("button", { name: "記録1を開く" }).click();
  await expect
    .element(page.getByRole("heading", { name: "記録1の詳細・編集" }))
    .toBeVisible();
  await page.getByLabelText("内容", { exact: true }).fill("パンを買う");
  await page.getByRole("button", { name: "記録を保存", exact: true }).click();
  await expect
    .element(page.getByRole("cell", { name: "パンを買う", exact: true }))
    .toBeVisible();
  await page.getByRole("button", { name: "記録1を再利用" }).click();
  await page.getByLabelText("内容", { exact: true }).fill("果物を買う");
  await page.getByRole("button", { name: "記録を保存", exact: true }).click();
  await expect
    .element(page.getByRole("cell", { name: "果物を買う", exact: true }))
    .toBeVisible();
...
test("FRM-010 テンプレートと複製で元のアプリ定義を変更しない", async () => {
  ...
  await page.getByRole("button", { name: "変更を反映", exact: true }).click();
  await page
    .getByRole("button", { name: "♧ 暮らしの記録", exact: true })
    .click();
...
test("FRM-040 通信失敗で入力を失わず再試行できる", async () => {
  ...
  const client = createClient(async (input, init) => {
    ...
    return api.request(new Request(input, init));
  }, "http://localhost");
  ...
  await expect
    ...
    .toBeVisible();
  await expect
    .element(page.getByLabelText("アプリ名", { exact: true }))
    .toHaveValue("入力を残す");
  await page.getByRole("button", { name: "アプリを作成", exact: true }).click();
  await expect
    .element(page.getByRole("button", { name: "変更を反映", exact: true }))
    .toBeVisible();
});
