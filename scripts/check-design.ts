...
const tokens = new Map([...css.matchAll(/(--[\w-]+):\s*([^;]+);/g)].map(([, key, value]) => [key, value.trim()]));
const errors: string[] = [];
for (const [, key] of `${css}\n${components}`.matchAll(/var\((--[\w-]+)\)/g)) {
  if (!tokens.has(key)) errors.push(`Unknown token: ${key}`);
}
if (/#[0-9a-f]{3,8}\b|\b(?:rgb|hsl)a?\(/i.test(components)) errors.push('Component colors must come from tokens');
if (!html.includes('href="components.css"')) errors.push('Preview must use shared components');
if (/style=|<style[\s>]/i.test(html)) errors.push('Preview must not redefine styles');
...
const contrasts = pairs.map(([foreground, background, minimum]) => {
  ...
  if (ratio < minimum) errors.push(`${foreground} on ${background}: ${ratio} < ${minimum}`);
  return { foreground, background, ratio: Number(ratio.toFixed(2)), minimum };
});
process.stdout.write(`${JSON.stringify({ ok: !errors.length, tokens: tokens.size, contrasts, errors }, null, 2)}\n`);
