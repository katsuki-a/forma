...
export type CheckResult = {
  ...
  documents: number;
  requirements: number;
  intentValidation: "passed" | "failed" | "not-run";
  errors: string[];
};
...
export function checkDocuments(
  ...
  const files = new Map(
    [...input].filter(([path]) => options.local || !path.startsWith("docs/")),
  );
  const errors: string[] = [];
  ...
  for (const [path, raw] of files) {
    ...
    for (const line of (header?.[1] ?? "").split("\n").filter(Boolean)) {
      ...
      if (!entry || entry[1] in meta)
        errors.push(`${path}: invalid or repeated metadata`);
    ...
    documents.push(doc);
    if (!/^# .+/m.test(body) && !path.startsWith(".github/"))
      errors.push(`${path}: title missing`);

    const area = path.split("/")[0];
    const managed =
      ["docs", "spec", "adr"].includes(area) &&
      posix.basename(path) !== "README.md";
    if (managed) {
      const prefix = { docs: "INT", spec: "SPEC", adr: "ADR" }[area];
      if (!new RegExp(`^${prefix}-[0-9]{3,}$`).test(meta.id ?? ""))
        errors.push(`${path}: invalid document id`);
      if (!/^\d{4}-[a-z0-9-]+\.md$/.test(posix.basename(path)))
        errors.push(`${path}: invalid filename`);
      const states =
        area === "spec"
          ? ["active"]
          : ["proposed", "accepted", "rejected", "superseded"];
      if (!states.includes(meta.status)) errors.push(`${path}: invalid status`);
      if (area !== "spec" && !/^\d{4}-\d{2}-\d{2}$/.test(meta.date ?? ""))
        errors.push(`${path}: date missing`);
      if (
        area === "docs" &&
        meta.status === "accepted" &&
        (meta["approved-by"] !== "PdM" || !meta["approval-evidence"])
      )
        errors.push(`${path}: approval evidence missing`);
      if (meta.id) {
        if (ids.has(meta.id))
          errors.push(`${path}: duplicate document id ${meta.id}`);
    ...
    if (
      ...
      /kintone|キントーン|学習のため|勉強のため/i.test(raw)
    ) {
      errors.push(
        `${path}: product comparison or personal motivation belongs in docs/`,
      );
    }
    for (const link of body
      ...
      try {
        ...
        errors.push(`${path}: malformed link ${target}`);
        continue;
      }
      const dest = posix
        .normalize(posix.join(posix.dirname(path), decoded))
        .replace(/\/$/, "");
      if (posix.isAbsolute(decoded) || !paths.has(dest))
        errors.push(`${path}: missing or non-relative link ${target}`);
    }
  }
  ...
  for (const doc of documents) {
    if (doc.meta.status === "superseded") {
      ...
      if (
        !successor ||
        successor === doc ||
        successor.meta.id.split("-")[0] !== doc.meta.id?.split("-")[0]
      ) {
        errors.push(`${doc.path}: valid successor missing`);
      }
    }
    if (
      !doc.path.startsWith("spec/") ||
      posix.basename(doc.path) === "README.md"
    )
      continue;
    ...
    for (const section of sections) {
      ...
      if (!id) {
        errors.push(`${doc.path}: invalid requirement heading`);
        continue;
      }
      if (requirements.has(id))
        errors.push(`${doc.path}: duplicate requirement id ${id}`);
      requirements.add(id);
      if (!/^確認条件: \S.+/m.test(section))
        errors.push(`${id}: acceptance criteria missing`);
  ...
  return {
    ...
    intentValidation: options.local
      ? errors.length
        ? "failed"
        : "passed"
      : "not-run",
...
function readRepository(
  ...
  for (const entry of readdirSync(resolve(root, dir), {
    ...
    if (entry.isDirectory()) {
      for (const [name, content] of readRepository(root, local, path))
